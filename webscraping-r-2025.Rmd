...
Let's extract the names of the distilleries by selecting the HTML node.

```{r Get Names Distilleries}
DistNames <- read_html(whiskypage) %>%
    html_nodes('.name') %>%
    html_text()
...
This time, lets  write a function to map over the webpage that scrapes the rating for each distillery using the HTML nodes:
...
We will need to flatten the object:
